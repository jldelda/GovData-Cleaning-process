...
df_IND = df_IND %>% mutate(Indicator = paste(Indicator, Dataset, "1 Yes 0 No", sep = "-"))

# # Indicator in df
df$Indicator = tolower(df$question)
df$Indicator = trimws(df$Indicator, which = 'both')

df = df %>% mutate(Indicator = paste(Indicator, dataset, "1 Yes 0 No", sep = "-"))
...
It seems that for WBL, 3 questions have been removed: 
- "what percentage of wages is paid during parental leave?-WBL-1 Yes 0 No"
- "what percentage of wages is paid during paternity leave?-WBL-1 Yes 0 No"
- "if there is a non-discrimination clause in the constitution does it mention gender?-WBL-1 Yes 0 No

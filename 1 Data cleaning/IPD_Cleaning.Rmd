...
df_IND = df_IND %>% mutate(Indicator_Id = paste(Display_Name_number, Dataset, tolower(answer), sep = "-"))
...
df_IND = df_IND %>% mutate(Indicator = paste(Indicator, Dataset, tolower(answer), sep = "-"))

# # Indicator in df
df$Indicator = tolower(df$question)
df$Indicator = gsub("[[:punct:]]","", df$Indicator)
#df_IND$Display_Name = trimws(df_IND$Display_Name, which = 'both')
df$Indicator = trimws(df$Indicator, which = 'both') 

df = df %>% mutate(Indicator = paste(Indicator, dataset, tolower(answer), sep = "-"))
...
Hierarchy$Indicator = gsub("visvis", "visàvis", Hierarchy$Indicator)
Hierarchy$Indicator = trimws(Hierarchy$Indicator, which = 'both')

Hierarchy = Hierarchy %>% mutate(Indicator = paste(Indicator, Dataset, tolower(Value.Type.Slug), sep = "-"))

...
# Drop unused columns
dropIND = c("Rank", "Category_11",  "Category_12",  "Category_13", "Category_14", "Category_21",  "Category_22", "Category_23", "Category_24" ,"Category_25", "Category_31", "Category_32", "Category_33", "Category_34",  "New1", "New2", "New3","New4", "New5", "keyN")
df_IND = df_IND[ ,!(names(df_IND) %in% dropIND)]

drop = c('keyN', 'Indicator_Id', 'Category_15')
...
#Hierarchy = read.csv("../Data/GovData360IND.csv", sep = ";")
...
Hierarchy$Display.Name = gsub("(BTI)", "", Hierarchy$Display.Name)  
Hierarchy$Display.Name = gsub("([&])[[:punct:]]", "", Hierarchy$Display.Name)
Hierarchy$Display.Name = gsub("  ", "", Hierarchy$Display.Name)
Hierarchy$Display.Name = trimws(Hierarchy$Display.Name, which = 'both')

Hierarchy = Hierarchy %>% mutate(Indicator = paste(Display.Name, Dataset, "Total", sep = "-"))
print(Hierarchy)
...
```
The indicators that do not match, will have the categories and some other columns empty in the final dataset.
**Step 5:** Left join between df_IND and Hierarchy.

```{r}
Hierarchy = Hierarchy %>% select('Indicator', 'Display.Name', 'Value.Type.Slug','Value.Type.Descriptor', 'Units', 'Rank.1', "Category.1.1", "Category.1.2", "Category.1.3", "Category.1.4", "Rank.2", "Category.2.1", "Category.2.2", "Category.2.3", "Category.2.4", "Category.2.5", "Rank.3","Category.3.1", "Category.3.2", "Category.3.3", "Category.3.4", "Indicator.Code", "Indicator.ID")
...
setdiff(df_IND$Indicator_Id, df$Indicator_Id)

# Which indicators of df are not in df_IND
setdiff(df$Indicator_Id, df_IND$Indicator_Id)
